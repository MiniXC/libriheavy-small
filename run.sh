...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  for subset in small dev test_clean test_other test_clean_large test_other_large; do
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  for subset in small test_clean test_other dev test_clean_large test_other_large; do
